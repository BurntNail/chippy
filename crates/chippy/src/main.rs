...
where F: Future<Output = tokio::io::Result<T>>
{
    match f.await {
        Ok(n) => Some(n),
...
async fn main() -> color_eyre::Result<()> {
        ...
        async move {
            ...
            loop {
                match tokio::io::stdin().read(&mut input).await {
                    Ok(n) => {
                        match String::from_utf8(input[0..n].to_vec()) {
                            Ok(message) => {
            ...
            loop {
                match message_decoder.wants_read() {
                    ...
                    DesiredInput::ProcessMe => {
                        message_decoder = match message_decoder.process().unwrap() { //FIXME: unwrap
                            FsmResult::Continue(decoder) => decoder,
                            FsmResult::Done(event) => {
                                match event {
                                    EventToClient::TxtSent {
                                        name, content
                                    } => {
    ...
    let _ = read_half.await.unwrap();
    let _ = write_half.await.unwrap();
